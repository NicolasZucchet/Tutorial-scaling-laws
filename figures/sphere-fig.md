<!-- Three unit spheres.  The middle one is the query case, and its four arrows
     ...
     colours, translated by the difference between the two circles' centres
     (cx 590 / cy 148 here, cx 880 / cy 167 there -- that figure's circle is its
     last panel, so it sits right of centre and the two do not overlap on
     screen).  The two slides are still one example seen twice -- embeddings
     built, then queried -- so if you move an arrow here, move the matching one
     there. -->
