<!-- Assumption 2, in three beats.  The slide is spoken left to right -- here are
     the contexts, each one gets a random embedding, out comes a next-token
     distribution -- so the figure is revealed the same way: the contexts are on
     screen from the start, step 1 draws the encoder and the unit circle, step 2
     the prediction.  The markers are `data-colloquium-fragment="1"` (never a
     hand-written index) so colloquium counts them and the arrow key can reach
     them; step 1 is the beat the slide's "uniformly on the unit sphere"
     sentence is pinned to, by an explicit index in slides.md.  (Spelling that
     attribute out here, digits and all, would make `--check` read this comment
     as a hand-written reveal, so it is only described.) -->
